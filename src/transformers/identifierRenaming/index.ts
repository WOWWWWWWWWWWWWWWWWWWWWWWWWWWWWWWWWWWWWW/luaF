...
export default function (root: Block): Block {
    ...
    */
	...
	function varIndexToValidVarName(options: Options, i: number): string {
		...
		if (!indexToVarName)
			throw new Error(`Renamer "${opts.mode}" does not exist.`)

		let name = varNamesLazy[i]
		if (!name) {
			do {
				name =
					opts.prefix + indexToVarName(nextValidNameIndex++, opts.increment)
	...
	for (let j = 0; j < allVariables.length; j++) {
		...
		let i = 0
		while (v.usedNameArray[i]) {
			i++
		}

		const opts = v.options
		if (!opts || !opts.identifierRenaming.enabled) continue
		...
		if (v instanceof Local) {
            ...
            */
			for (const otherVar of allVariables.slice(j + 1)) {
				if (
					!(otherVar instanceof Local) ||
					otherVar.scope.depth < v.scope.depth
				) {
					/*  
                    ...
                    */
					...
					// otherVar.scope.depth must be equal to v.scope.depth
					/*
                        Check case 1
                    ...
                    */
					if (
						v.beginLocation < otherVar.endLocation &&
						v.endLocation > otherVar.beginLocation
					) {
						otherVar.usedNameArray[i] = true
					}
				}
