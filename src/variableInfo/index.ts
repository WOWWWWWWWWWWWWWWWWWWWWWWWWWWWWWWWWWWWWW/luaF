import { Block, Statement } from "../parser/types/Base.ts"
import { VariableExpr } from "../parser/types/expressions/VariableExpr.ts";
import { TokenType } from "../parser/types/Token.ts";
import { Walker } from "../parser/types/Walker.ts";
import { Scope } from "./Scope.ts";
import { Global, Local, LocalType, NameFunc, Variable, VariableInfo } from "./Variable.ts";

// Numbering generator for variable lifetimes
let locationGenerator = 0
export const markLocation = () => locationGenerator++

export function createVariableInfo(root: Block): [globals: Global[], rootScope: Scope] {
    locationGenerator = 0
    ...
    visitor.variableExpr = {
        ...
        */
        enter: expr => {
            ...
            const v: Variable = currentScope.getVariable(name) || getGlobalVar(name, null)
        ...
        enter: stat => {
            ...
            */
            if (stat.local) {
                ...
                const v: Variable = currentScope.getVariable(name) || getGlobalVar(name, stat)
