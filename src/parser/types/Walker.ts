import { Node, Block, Statement, Expression } from "./Base.ts";
import { Options } from "./Context.ts";

import { IfStat } from "./statements/IfStat.ts";
import { WhileStat } from "./statements/WhileStat.ts";
import { DoStat } from "./statements/DoStat.ts";
import { GenericForStat, NumericForStat } from "./statements/ForStat.ts";
import { RepeatStat } from "./statements/RepeatStat.ts";
import { FunctionStat } from "./statements/FunctionStat.ts";
import { LocalVarStat } from "./statements/LocalVarStat.ts";
import { ReturnStat } from "./statements/ReturnStat.ts";
import { BreakStat } from "./statements/BreakStat.ts";
import { CallExprStat } from "./statements/CallExprStat.ts";
import { AssignmentStat } from './statements/AssignmentStat.ts';
import { BinopExpr } from "./expressions/BinopExpr.ts";
import { UnopExpr } from "./expressions/UnopExpr.ts";
import { NumberLiteral } from "./expressions/NumberLiteral.ts";
import { StringLiteral } from "./expressions/StringLiteral.ts";
import { NilLiteral } from "./expressions/NilLiteral.ts";
import { BooleanLiteral } from "./expressions/BooleanLiteral.ts";
import { VargLiteral } from "./expressions/VargLiteral.ts";
import { FieldExpr } from "./expressions/FieldExpr.ts";
import { IndexExpr } from "./expressions/IndexExpr.ts";
import { ArgCall, StringCall, TableCall } from "./expressions/CallExpr.ts";
import { FunctionLiteral } from "./expressions/FunctionLiteral.ts";
import { VariableExpr } from "./expressions/VariableExpr.ts";
import { ParenExpr } from "./expressions/ParenExpr.ts";
import { TableField, TableIndex, TableLiteral, TableValue } from "./expressions/TableLiteral.ts";
...
type VisitorFunction<T extends Node, A> = (
    ...
    ...a: ( // https://stackoverflow.com/questions/52318011/optional-parameters-based-on-conditional-types
        T extends Block ? [] :
        T extends Statement ? [block: Block] :
        T extends Expression ? [statement: Statement, block: Block] : [unknown]
    )
) => A | void
...
export class Walker {
    ...
    private visit<T extends Node, A>(visitor: Visitor<T, A>, mutate: (out: A) => T | void): [enter: VisitorFunction<T, A>, leave: VisitorFunction<T, A>] {
        return [
            (...a) => {
                if (visitor.enter) {
    ...
    private _block(block: Block) {
        ...
        for (let i = 0; i < block.stats.length; i++) {
            const [senter, sleave] = this.visit(this.statement, s => block.stats[i] = s)
    ...
    private _statement(statement: Statement, block: Block): Statement {
        ...
        if (visitor) {
            ...
            if (visitChildren)
                visitChildren.call(this, statement, block)
    ...
    private _expression(expression: Expression, statement: Statement, block: Block): Expression {
        // deno-lint-ignore no-explicit-any
        let visitor: Visitor<any, Expression> | undefined;

        // deno-lint-ignore no-explicit-any
        let visitChildren: ((expression: any, statement: Statement, block: Block) => void) | undefined
        ...
        if (visitor) {
            ...
            if (visitChildren)
                visitChildren.call(this, expression, statement, block)
    ...
    private _ifStat(statement: IfStat, block: Block) {
        statement.condition = this._expression(statement.condition, statement, block)
        ...
        for (let i = 0; i < statement.elseClauses.length; i++) {
            ...
            if (elseClause.condition)
                statement.elseClauses[i].condition = this._expression(elseClause.condition, statement, block)
    ...
    private _doStat(statement: DoStat, block: Block) {
        statement.body = this._block(statement.body)
    }

    private _numericForStat(statement: NumericForStat, block: Block) {
        statement.rangelist = statement.rangelist.map(e => this._expression(e, statement, block))
        statement.body = this._block(statement.body)
    }

    private _genericForStat(statement: GenericForStat, block: Block) {
        statement.generatorlist = statement.generatorlist.map(e => this._expression(e, statement, block))
        statement.body = this._block(statement.body)
    }
    ...
    private _functionStat(statement: FunctionStat, block: Block) {
        statement.body = this._block(statement.body)
    }

    private _localVarStat(statement: LocalVarStat, block: Block) {
        statement.rhs = statement.rhs.map(e => this._expression(e, statement, block))
    }

    private _returnStat(statement: ReturnStat, block: Block) {
        statement.list = statement.list.map(e => this._expression(e, statement, block))
    }
    ...
    private _assignmentStat(statement: AssignmentStat, block: Block) {
        statement.lhs = statement.lhs.map(e => this._expression(e, statement, block))
        statement.rhs = statement.rhs.map(e => this._expression(e, statement, block))
    }

    // Expressions

    private _binopExpr(expression: BinopExpr, statement: Statement, block: Block) {
    ...
    private _fieldExpr(expression: FieldExpr, statement: Statement, block: Block) {
        expression.base = this._expression(expression.base, statement, block)
    }

    private _indexExpr(expression: IndexExpr, statement: Statement, block: Block) {
        expression.base = this._expression(expression.base, statement, block)
    ...
    private _argCall(expression: ArgCall, statement: Statement, block: Block) {
        expression.base = this._expression(expression.base, statement, block)
        expression.arguments = expression.arguments.map(e => this._expression(e, statement, block))
    }

    private _stringCall(expression: StringCall, statement: Statement, block: Block) {
        expression.base = this._expression(expression.base, statement, block)
    }

    private _tableCall(expression: TableCall, statement: Statement, block: Block) {
        expression.base = this._expression(expression.base, statement, block)

        const expr = <TableLiteral>this._expression(expression.table, statement, block)
        expression.table = expr
    }

    private _functionLiteral(expression: FunctionLiteral, statement: Statement, block: Block) {
        expression.body = this._block(expression.body)
    }

    private _parenExpr(expression: ParenExpr, statement: Statement, block: Block) {
        expression.value = this._expression(expression.value, statement, block)
    }

    private _tableLiteral(expression: TableLiteral, statement: Statement, block: Block) {
