import {
	MatchEvenBackslashes,
	MatchOddBackslashes,
	Keywords,
	Symbols
} from "@utils/constants"
import { StreamedToken, TokenType } from "@ast/Token"
...
export function lexer(input: string): StreamedToken[] {
	...
	const match = (r: RegExp) =>
		input.substring(p).match(new RegExp(`^(?:${r.source})`, r.flags))
	...
	while (true) {
		...
		while (true) {
			...
			if (p >= length) {
				...
				let commentEnd
				if (m[1]) {
					// Long comment if there is a first group
					...
					m1
						? (p += m1[0].length)
						: panic("something happened while consuming normal comment")
					commentEnd = p
				}
		...
		if (p >= length) {
			...
			(m = match(
				new RegExp(
					`"(.*?${MatchEvenBackslashes.source})"` +
						"|" +
						`'(.*?${MatchEvenBackslashes.source})'`
				)
			))
		) {
			// match quote with next quote with an even number of preceding backslashes (including zero)
			// Get invalid letter escapes
			const content = m[1] || m[2]
			if (content) {
				for (const invalid of content.matchAll(
					new RegExp(
						`${MatchOddBackslashes.source}${/([^\drnt"'\\])/.source}`,
						"g"
					)
				)) {
